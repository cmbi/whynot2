...
public abstract class Connector {
	...
	public static void connect(Properties dbconf) throws DBConnectionException {
		...
		try {
			...
			Connector.dbconnection = DriverManager.getConnection(dbconf
					.getProperty("protocol")
					+ "//"
					+ dbconf.getProperty("host")
					+ ":"
					+ dbconf.getProperty("port")
					+ "/"
					+ dbconf.getProperty("name"), dbconf.getProperty("user"),
					dbconf.getProperty("pass"));

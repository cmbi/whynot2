...
public class EventManager {
		...
		if (args[0].equals("store"))
			...
			if (args[0].equals("list")) {
				...
				if (args[0].equals("addpersontoevent")) {
					...
					if (args[0].equals("addemailtoperson")) {
	...
	private void addPersonToEvent(Long personId, Long eventId) {
		...
		Event anEvent = (Event) session.createCriteria(Event.class).setFetchMode("participants", FetchMode.JOIN).add(Restrictions.eq("id", eventId)).uniqueResult(); // Eager fetch the colleciton so we can use it detached
